...
export default function SuperAdminRolesPage() {
  ...
  if (loading) {
    return <div className="p-4 md:p-8">Cargando roles...</div>
  }
  ...
  return (
    <div className="p-4 md:p-8 space-y-4 md:space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
        {roles.map((rol) => (
          <div key={rol.id} className="bg-white rounded-lg shadow p-4 md:p-6 space-y-3">
            <h3 className="text-lg md:text-xl font-semibold">{rol.nombre}</h3>
      ...
      {showModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 md:p-8 max-w-md w-full">
            <h2 className="text-xl md:text-2xl font-bold mb-4">{editando ? "Editar Rol" : "Nuevo Rol"}</h2>
